...
const Canvas = forwardRef(function Canvas({ tool, color, brushSize, image }, ref) {
  ...
  // ── Saving ─────────────────────────────────────────────────────
  const lastSavedHashRef = useRef(''); // avoid redundant writes
  ...
  const flushSave = useCallback(async () => {
    ...
    // Basic redundancy check (length is a cheap heuristic)
  ...
  // Flush on tab close / backgrounding (more reliable than unmount)
  useEffect(() => {
    const onVis = () => {
